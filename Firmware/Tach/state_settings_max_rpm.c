...
{	
	...
	switch (event)
	{
		...
		case TACH_EVENT_REDRAW_SCREEN:
			snprintf(pData->out_buf, DISPLAY_LINE_SIZE+1, (pData->view_mode == 1 ? "   %u מב/לטם" : "   <%u> מב/לטם"), pData->tmp_max_rpm_setting);
		...
		case TACH_EVENT_ENCODER_LEFT:
			...
			else
			{
				...
				if (pData->tmp_max_rpm_setting > 5) pData->tmp_max_rpm_setting -= 5;
			}			
			break;		
		default:
			break;				
